...
print(f"   - RMSE (Root Mean Squared Error): Rs.{train_rmse:.2f}")
print(f"   - MAE (Mean Absolute Error): Rs.{train_mae:.2f}")
print(f"   - R² Score (Accuracy): {train_r2:.4f} ({train_r2*100:.2f}%)")

print("\n   TEST SET PERFORMANCE:")
print(f"   - RMSE (Root Mean Squared Error): Rs.{test_rmse:.2f}")
print(f"   - MAE (Mean Absolute Error): Rs.{test_mae:.2f}")
...
axes[0, 1].set_xlabel('Prediction Error (Rs.)', fontsize=11)
...
axes[1, 0].set_ylabel('Price (Rs.)', fontsize=11)
...
print(f"  - Average Error (MAE): Rs.{test_mae:.2f}")
print(f"  - RMSE: Rs.{test_rmse:.2f}")
print(f"\nFiles saved:")
